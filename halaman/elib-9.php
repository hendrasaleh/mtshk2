    <div class="about home-about">
        <div class="container">
			<div class="row">
				<div class="col-md-12">
					<div class="section-title text-center">
						<h2>BUKU SEKOLAH ELEKTRONIK KELAS 9</h2>
					</div>
				</div>
			</div>
            <div class="row">
                <div id="isotope-gallery-container">
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/n.jpg" class="img-responsive" alt="3rd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/n.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                            	<div class="editContent">
                                    <h4>SKI</h4>
                                    <a href="index.php?page=ski-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=ski-9" class="btn btn-primary">Buku Guru</a>
                            	</div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/o.jpg" class="img-responsive" alt="1st gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/o.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>Qurdits</h4>
                                    <a href="index.php?page=qurdits-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=qurdits-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper rutin">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/p.jpg" class="img-responsive" alt="2nd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/p.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>PKn</h4>
                                    <a href="index.php?page=pkn-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=pkn-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/q.jpg" class="img-responsive" alt="3rd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/q.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>PJOK</h4>
                                    <a href="index.php?page=pjok-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=pjok-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper rutin">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/r.jpg" class="img-responsive" alt="2nd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/r.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>Matematika</h4>
                                    <a href="index.php?page=mtk-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=mtk-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/s.jpg" class="img-responsive" alt="3rd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/s.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>IPS</h4>
                                    <a href="index.php?page=ips-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=ips-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/t.jpg" class="img-responsive" alt="1st gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/t.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>IPA</h4>
                                    <a href="index.php?page=ipa-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=ipa-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/u.jpg" class="img-responsive" alt="3rd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/u.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>Fikih</h4>
                                    <a href="index.php?page=fikih-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=fikih-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/v.jpg" class="img-responsive" alt="1st gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/v.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>Bahasa Inggris</h4>
                                    <a href="index.php?page=inggris-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=inggris-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper rutin">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/w.jpg" class="img-responsive" alt="2nd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/w.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>Bahasa Indonesia</h4>
                                    <a href="index.php?page=indonesia-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=indonesia-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/x.jpg" class="img-responsive" alt="3rd gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/x.jpg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>Bahasa Arab</h4>
                                    <a href="index.php?page=arab-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=arab-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
                    <div class="col-md-4 col-sm-6 col-xs-12 gallery-item-wrapper club">
                        <div class="gallery-item">
                            <div class="gallery-thumb">
                                <img src="img/book/y.jpg" class="img-responsive" alt="1st gallery Thumb">
                                <div class="image-overlay"></div>
                                <a href="img/book/y.jpeg" class="gallery-zoom"><i class="fa fa-eye"></i></a>
                            </div>
                            <div class="gallery-details">
                                <div class="editContent">
                                    <h4>Akidah Akhlak</h4>
                                    <a href="index.php?page=akidah-9" class="btn btn-primary">Buku Siswa</a>
                                    <a href="index.php?page=akidah-9" class="btn btn-primary">Buku Guru</a>
                                </div>
                            </br>
                            </div>
                        </div>
                    </div>
                    <!-- /.gallery-item-wrapper -->
		</div>
	</section>	  